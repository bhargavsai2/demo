(function($){
    ...
    function init_carousel_mobile(){
        ...
        window_size += croche_get_scrollbar_width();
        if( window_size < 768 ){
    ...
    /* ---------------------------------------------
     ...
     function init_menu_reposive(){
          ...
          if(kt_is_mobile === true){
            $(document).on('click', '.croche-nav .menu-item-has-children > a', function(e){
              var licurrent = $(this).closest('li');
              var liItem = $('.croche-nav .menu-item-has-children');
        ...
        $(document).on('click', function(e){
              var target = $( e.target );
              if ( !target.closest('.show-submenu').length || !target.closest('.croche-nav').length ) {
                  $('.show-submenu').removeClass('show-submenu');
              }
          }); 
          // On Desktop
          }else{
              $(document).on('mousemove','.croche-nav .menu-item-has-children',function(){
                  $(this).addClass('show-submenu');
              })
              $(document).on('mouseout','.croche-nav .menu-item-has-children',function(){
    ...
    /* ---------------------------------------------
     Resize mega menu
     --------------------------------------------- */
     function croche_resizeMegamenu(){
        var window_size = jQuery('body').innerWidth();
        window_size += croche_get_scrollbar_width();
        if( window_size > 767 ){
     ...
     function croche_get_scrollbar_width() {
    ...
    /* ---------------------------------------------
     ...
     --------------------------------------------- */
    function init_popup(){
        if($(window).width() + croche_get_scrollbar_width() >= 768){
            if($('body').hasClass('home')){
                //Open directly via API
                $.magnificPopup.open({
                  items: {
                    src: '<div class="white-popup"><div class="kt-popup-newsletter"><div class="popup-title"><h3>croche</h3><img src="images/arow-popup.png" alt=""><p class="notice">enter your email and get  <span class="primary">25% off</span> YOUR first purchase!</p></div><form class="form-subscribe"><input class="input" placeholder="Your email here" type="text" /><button class="button">NO THANKS!</button><button class="button">Enter</button></form><div class="checkbox"><label><input type="checkbox" value="">Dont show this popup again!</label></div></div></div>',  //can be a HTML string, jQuery object, or CSS selector
                    type: 'inline'
                  }
                });
            }
...
s1.async=true;
s1.src='https://embed.tawk.to/5e862c8e69e9320caabfad24/default';
s1.charset='UTF-8';
